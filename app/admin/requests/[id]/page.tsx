...
import { UpdateRequestStatus } from "@/components/requests/update-request-status";
...
export default async function AdminRequestDetailPage({
  ...
  const { data: row, error } = await supabase
    ...
    .select(
      "id, title, body, status, created_at, updated_at, units(flat_number, owner_name)"
    )
    .eq("id", id)
    .single();
  ...
  const canUpdate =
    isCommittee(resident.role) || isFacilityManager(resident.role);
      ...
      <div>
        ...
        <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
          ...
          {canUpdate ? (
            <UpdateRequestStatus
              requestId={row.id}
              current={row.status as ResidentRequestStatus}
            />
          ) : (
